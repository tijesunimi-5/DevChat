...
const Registration = () => {
  ...
  const { user, setUser, signedIn, setSignedIn, setLinkUploadProgress } = useUser();
  ...
  }, [user, signedIn, setLinkUploadProgress]);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    setUser(userData);
    setSignedIn(true); // Update signedIn state
    localStorage.setItem('user', JSON.stringify(userData)); // Save to localStorage
    localStorage.setItem('signedIn', 'true'); // Save signedIn status
    increaseProgress();
  ...
  return (
    <div className="pt-12 px-2 relative pb-8">
      ...
      <form
        className="mt-5 rounded-2xl regShad p-5 overflow-hidden relative flex flex-col justify-between"
        onSubmit={handleSubmit}
      >
        <div className="progess h-2 absolute top-0 left-0 right-0 rounded-2xl w-full">
          <span
            className="bg-[#3D3C99] absolute h-2 rounded-2xl"
        ...
        <p className="mt-3 z-20">
          ...
          <Link href="/Registration/login" className="cursor-pointer underline">
            Sign In
          </Link>
        </p>
      </form>
